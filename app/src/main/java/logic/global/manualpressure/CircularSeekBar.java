...
public class CircularSeekBar extends View {
    ...
    // Default values
    protected static final float DEFAULT_SEMI_CIRCLE_RADIUS = 3f; //hrubka polkruznice okolo textu s percentami
    protected static final float DEFAULT_CIRCLE_X_RADIUS = 18f; //modra gulicka
    protected static final float DEFAULT_CIRCLE_Y_RADIUS = 18f; //modra gulicka
    protected static final float DEFAULT_POINTER_RADIUS = 12f; //velkost velkej polkruznice
    ...
    protected static final float DEFAULT_CIRCLE_STROKE_WIDTH = 9f;
    ...
    //My variables
    protected float semiCircleStrokeWidth; //hrubka malej polkruznice
    protected  Paint semiCirclePaint;
    protected  float semiCircleKoef=2.8f;
    protected  float trianglePositionKoef=2.3f;
    protected float triangleKoef = 3f;
    protected float margin = 15f;
    ...
    protected void initAttributes(TypedArray attrArray) {
        margin = margin * DPTOPX_SCALE;
        semiCircleStrokeWidth = DEFAULT_SEMI_CIRCLE_RADIUS * DPTOPX_SCALE;
    ...
    protected void initPaints() {
        ...
        textPaint.setColor(Color.parseColor("#3e5b71"));

        semiCirclePaint = new Paint();
        semiCirclePaint.setColor(Color.parseColor("#bed1d5"));
        semiCirclePaint.setStrokeWidth(semiCircleStrokeWidth);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //litle semiCircle
        float semiCircle = mCircleWidth/semiCircleKoef;
        RectF oval = new RectF(-semiCircle, -semiCircle, semiCircle, semiCircle);
        canvas.drawArc(oval, mStartAngle, 180, false, semiCirclePaint);
        //big colored semiCircle
        mCirclePaint.setColor(Color.parseColor("#e1000e"));
        canvas.drawArc(mCircleRectF, mStartAngle, 22.5f, false, mCirclePaint);
        mCirclePaint.setColor(Color.parseColor("#fdd600"));
        canvas.drawArc(mCircleRectF, -157.5f, 22.5f, false, mCirclePaint);
        mCirclePaint.setColor(Color.parseColor("#becd00"));
        canvas.drawArc(mCircleRectF, -135f, 45f, false, mCirclePaint);
        mCirclePaint.setStyle(Paint.Style.STROKE);
        mCirclePaint.setColor(Color.parseColor("#bed1d5"));
        mCirclePaint.setStrokeWidth(semiCircleStrokeWidth);
        canvas.drawArc(mCircleRectF, -90f, 90f, false, mCirclePaint);
        mCirclePaint.setStrokeWidth(mCircleStrokeWidth);

        //text with progress in percents
        textPaint.setTextSize(mCircleWidth / 5);
        canvas.drawText(mProgress + "%", 0, 0, textPaint);

        //labels
        textPaint.setTextSize(mCircleWidth / 9);
        System.out.println("polomer:" + (float) ((mCircleWidth) * Math.cos(180 * Math.PI / 180)));
        canvas.drawText("0", (float) ((mCircleWidth + 25 * DPTOPX_SCALE) * Math.cos(180 * Math.PI / 180)), 0, textPaint);
        canvas.drawText("1/4", (float) ((mCircleWidth+25*DPTOPX_SCALE) * Math.cos(202.5* Math.PI / 180)), (float) ((mCircleWidth+15*DPTOPX_SCALE) * Math.sin(202.5 * Math.PI / 180)), textPaint);
        canvas.drawText("1/2", (float) ((mCircleWidth+25*DPTOPX_SCALE) * Math.cos(225* Math.PI / 180)), (float) ((mCircleWidth+15*DPTOPX_SCALE) * Math.sin(225 * Math.PI / 180)), textPaint);
        canvas.drawText("3/4", (float) ((mCircleWidth+25*DPTOPX_SCALE) * Math.cos(247.5* Math.PI / 180)), (float) ((mCircleWidth+15*DPTOPX_SCALE) * Math.sin(247.5 * Math.PI / 180)), textPaint);
        canvas.drawText("Full", (float) ((mCircleWidth + 20 * DPTOPX_SCALE) * Math.cos(270 * Math.PI / 180)), (float) ((mCircleWidth + 15 * DPTOPX_SCALE) * Math.sin(270 * Math.PI / 180)), textPaint);
        ...
        //triangle
        Paint trinaglePaint = new Paint();
        trinaglePaint.setColor(android.graphics.Color.parseColor("#bed1d5"));
        trinaglePaint.setStyle(Paint.Style.FILL);
        trinaglePaint.setAntiAlias(true);
        float triangleWidth = mCircleWidth/triangleKoef;
        int triangleRaduis = (int) (triangleWidth/4.5);
        Point a = new Point(0, 0);
        Point b = new Point(0, triangleRaduis);
        Point c = new Point((int) triangleWidth, 0);
        Point d = new Point(0, -triangleRaduis);
        Path path = new Path();
        path.moveTo(a.x, a.y);
        path.lineTo(b.x, b.y);
        path.lineTo(c.x, c.y);
        ...
        path.lineTo(a.x, a.y);
        path.close();
        canvas.translate(mPointerPositionXY[0] / trianglePositionKoef, mPointerPositionXY[1] / trianglePositionKoef);
        canvas.rotate(mPointerPosition);
        canvas.drawPath(path, trinaglePaint);
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mCircleHeight = (float)height / 2f - mCircleStrokeWidth - mPointerRadius - (mPointerHaloBorderWidth * 2.0f)-margin;
        mCircleWidth = (float)width / 2f - mCircleStrokeWidth - mPointerRadius - (mPointerHaloBorderWidth * 2.0f)-margin;
        ...
        if (mCustomRadii) {
            ...
            if ((mCircleYRadius - mCircleStrokeWidth - mPointerRadius - mPointerHaloBorderWidth) < mCircleHeight) {
                mCircleHeight = mCircleYRadius - mCircleStrokeWidth - mPointerRadius - (mPointerHaloBorderWidth * 2.0f);
            }

            if ((mCircleXRadius - mCircleStrokeWidth - mPointerRadius - mPointerHaloBorderWidth) < mCircleWidth) {
                mCircleWidth = mCircleXRadius - mCircleStrokeWidth - mPointerRadius - (mPointerHaloBorderWidth * 2.0f);
            }
        }
